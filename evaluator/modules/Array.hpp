	...
	public:
		...
		ArrayModule() {
				...
				std::make_shared<FunctionValue>(
					[](std::vector<std::shared_ptr<Value>>& args) {
						...
						for (unsigned int i = 0; i < arr->size(); ++i) {
							...
							results.push_back(callback->run(args));
						}
			...
			variables.insert({
				"find",
				std::make_shared<FunctionValue>(
					[](std::vector<std::shared_ptr<Value>>& args) -> std::shared_ptr<Value> {
						const auto arr = valueCast<ArrayValue>(args[0]);
						const auto callback = valueCast<FunctionValue>(args[1]);
				...
				std::make_shared<FunctionValue>(
					[](std::vector<std::shared_ptr<Value>>& args) {
						...
						std::sort(
							...
							[&compare](std::shared_ptr<Value> a, std::shared_ptr<Value> b) {
								std::vector<std::shared_ptr<Value>> args{a, b};
								return valueCast<BooleanValue>(compare->run(args))->value;
							}
						);
						return arr;
					}
